...
describe('Check DFS/BFS functions', () => {
    ...
    test('check getGenerators', () => {
        expect(eg.getGenerators()).toEqual([g1, g2, g4, g3]);
    });
    test('check getConnectionsNode', () => {
        expect(eg.getConnectionsNode(eg.tree[0].out[0])).toEqual(eg.tree[0].out[0].element.out_connections);
    });
});
